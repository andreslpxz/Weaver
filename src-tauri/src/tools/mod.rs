...
pub struct FileWriteArgs {
    ...
    /// Si es true, `content` viene codificado en base64 (sin prefijo data:)
    /// y debe decodificarse a bytes antes de escribir. Necesario para
    /// archivos binarios reales (audio/imagen/video) — escribir base64
    /// como texto plano corrompe el archivo.
...
pub async fn tools_file_write(args: FileWriteArgs) -> Result<(), String> {
    ...
    if args.is_base64.unwrap_or(false) {
        use base64::Engine;
        let bytes = base64::engine::general_purpose::STANDARD
            .decode(args.content.trim())
