...
interface WeaverState {
   ...
   * limpias (spawn_child_agent), y puede auto-refinarse (/refine).
   */
  rlmEnabled: boolean;
  /** Spawns de subagentes RLM activos (para UI). */
  rlmSpawns: import('@/agent/rlm').SpawnInfo[];
  /** Resultados de subagentes RLM por childId. */
  rlmResults: Record<string, import('@/agent/rlm').SpawnResult>;
  /** Fragmentos del ContextStore activo. */
  rlmFragments: import('@/agent/rlm').ContextFragment[];
  /** Tamaño total del ContextStore. */
  rlmTotalFragmentSize: number;
  ...
  setRlmFragments: (fragments: import('@/agent/rlm').ContextFragment[], totalSize: number) => void;
  clearRlmState: () => void;
...
export const useWeaver = create<WeaverState>((set, get) => ({
  ...
  clearRlmState: () => set({ rlmSpawns: [], rlmResults: {}, rlmFragments: [], rlmTotalFragmentSize: 0 }),
